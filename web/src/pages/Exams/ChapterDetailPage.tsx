...
const API_ORIGIN = (import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/v1').replace(/\/api\/v\d+\/?$/, '')
...
function PdfViewer({ url, title }: { url: string; title: string }) {
  ...
  useEffect(() => {
    ...
    fetch(url, { credentials: 'include' }).then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.blob() }).then(blob => { objectUrl = URL.createObjectURL(blob); setBlobUrl(objectUrl) }).catch(err => setError(err.message)).finally(() => setLoading(false))
  ...
  return <iframe src={blobUrl!} title={title} style={{ width: '100%', height: '80vh', border: 'none', display: 'block' }} />
}
