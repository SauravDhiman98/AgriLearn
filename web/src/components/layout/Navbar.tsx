      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={handleSearch} className="md:hidden" style={{ color: isDark ? '#9ca3af' : '#6b7280', background: 'none', border: 'none', cursor: 'pointer' }}><Search className="w-5 h-5" /></button>
            <Link to="/marketplace" style={{ color: isDark ? '#9ca3af' : '#6b7280' }} className="relative hover:text-gray-900"><ShoppingCart className="w-5 h-5" />{cartCount > 0 && <span className="absolute -top-1.5 -right-1.5 bg-green-600 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">{cartCount}</span>}</Link>
            {isAuthenticated ? <div className="flex items-center gap-2"><Link to="/dashboard" className="hidden md:block btn-outline text-sm py-1.5">{t('nav.dashboard')}</Link><div className="relative" onMouseEnter={openProfile} onMouseLeave={closeProfileDelayed}><button className="flex items-center gap-1"><div className="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center text-green-700 font-semibold text-sm">{user?.firstName?.[0]}</div></button>{profileOpen && <div onMouseEnter={openProfile} onMouseLeave={closeProfileDelayed} style={{ backgroundColor: isDark ? '#1f2937' : '#ffffff', border: `1px solid ${isDark ? '#374151' : '#e5e7eb'}` }} className="absolute right-0 mt-2 w-48 rounded-lg shadow-lg py-1 z-50"><Link to="/profile" style={{ color: isDark ? '#d1d5db' : '#374151' }} className="block px-4 py-2 text-sm hover:bg-gray-100">{t('nav.profile')}</Link>{user?.role === 'ADMIN' && <><Link to="/admin" style={{ color: isDark ? '#d1d5db' : '#374151' }} className="block px-4 py-2 text-sm hover:bg-gray-100">Admin Dashboard</Link><Link to="/admin/analytics" style={{ color: isDark ? '#d1d5db' : '#374151' }} className="block px-4 py-2 text-sm hover:bg-gray-100">Analytics</Link><Link to="/admin/logs" style={{ color: isDark ? '#d1d5db' : '#374151' }} className="block px-4 py-2 text-sm hover:bg-gray-100">Log Viewer</Link></>}<hr style={{ borderColor: isDark ? '#374151' : '#f3f4f6' }} className="my-1" /><button onClick={() => { dispatch(logout()); queryClient.clear(); navigate('/') }} className="w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-gray-100">{t('nav.logout')}</button></div>}</div></div> : <div className="hidden md:flex items-center gap-2"><Link to="/login" style={{ color: isDark ? '#d1d5db' : '#4b5563' }} className="font-medium text-sm">{t('nav.login')}</Link><Link to="/register" className="btn-primary text-sm py-1.5">{t('nav.register')}</Link></div>}
